...
def Getmodel_tensorflow(nb_classes):  #设定字符分割网络，定义了一个三层的卷积神经网络，在TensorFlow中可运行
    ...
    model = Sequential()
    model.add(Conv2D(nb_filters, (nb_conv, nb_conv),input_shape=(img_rows, img_cols,1))) #前二层为卷积池化层，无正则化减少过拟合
    model.add(Activation('relu'))    #激活函数
    model.add(MaxPool2D(pool_size=(nb_pool, nb_pool)))    #最大池化
    model.add(Conv2D(nb_filters, (nb_conv, nb_conv)))     #第二次卷积，16个滤波器，尺寸为3*3
    model.add(Activation('relu'))    #激活函数
    model.add(MaxPool2D(pool_size=(nb_pool, nb_pool)))    #最大池化
    model.add(Flatten())             #归一化，转变为X*1维向量
    model.add(Dense(256))          #全连接层，输出一个256维向量
    model.add(Dropout(0.5))        #正则化，减少过拟合

    model.add(Activation('relu'))   #再次激活
    model.add(Dense(nb_classes))
    model.add(Activation('softmax'))   #最后一层为softmax多分类器
    model.compile(loss='categorical_crossentropy',  ##梯度下降，sgd优化算法
                  optimizer='sgd',
                  metrics=['accuracy'])
    return model
...
def Getmodel_tensorflow_light(nb_classes):   #设定轻量级字符分割网络，定义了一个三层的卷积神经网络，在TensorFlow中可运行，与上一个网络互补，可自行选用
    # nb_classes = len(charset)
    img_rows, img_cols = 23, 23
    # number of convolutional filters to use
    nb_filters = 8
...
import os
model.load_weights("./model/char_judgement1.h5")  #保存权重矩阵，生成h5文件
# model.save("./model/char_judgement1.h5")
model2.load_weights("./model/char_judgement.h5")  #保存权重矩阵，生成h5文件
...
def searchOptimalCuttingPoint(rgb,res_map,start,width_boundingbox,interval_range): #定义车牌最佳裁剪边框
    t0  = time.time()
    #
    # for x in xrange(10):
    ...
    if width_boundingbox>20:  #缩小边框尺寸
        refine_s = -9
    score_list = []
    interval_big = int(width_boundingbox * 0.3)  #缩小为0.3倍
    p = 0
...
def slidingWindowsEval(image): #定义滑动窗函数，滑动窗尺寸为16，步长为1
    windows_size = 16;
    stride = 1
    ...
    for i in range(0,image.shape[1]-windows_size+1,stride):  #遍历整张图片，使用滑动窗将图片完全分割，生成数据结果
        data = image[0:height,i:i+windows_size]
        data = cv2.resize(data,(23,23))  #重设图片尺寸
        # cv2.imshow("image",data)
        data = cv2.equalizeHist(data)
        data = data.astype(np.float)/255  #图片降维
