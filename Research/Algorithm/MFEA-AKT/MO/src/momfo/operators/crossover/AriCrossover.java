 ...
 * solutions.
 */
public class AriCrossover extends Crossover {
	/**
	...
	public AriCrossover(HashMap<String, Object> parameters) {
		super(parameters);
...
/*		int dem1=problemSet.get(parent1.getSkillFactor()).getNumberOfVariables();
		...
		if (PseudoRandom.randDouble() <= probability) {
			...
			for(i=0;i<dim;i++){
				...
				insertValue=lambda*x1.getValue(i)+(1-lambda)*x2.getValue(i);
				if (insertValue < yL)
					insertValue = yL;
				...
				insertValue=lambda*x2.getValue(i)+(1-lambda)*x1.getValue(i);
				if (insertValue < yL)
					insertValue = yL;
