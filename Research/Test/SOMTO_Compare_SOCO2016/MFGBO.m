    ...
    for rep = 1:reps
        ...
        while generation <= gen && TotalEvaluations(rep, generation) < eva_num
            ...
            for i = 1:pop
                ...
                % 生成新个体
                Chromosome_MFGBO_new = Chromosome_MFGBO();
                Chromosome_MFGBO_new.rnvec = Xnew;

                % 遗传主个体的技能因子，或变异
                if rand() < pm
                    % 变异
                    Chromosome_MFGBO_new.skill_factor = randi(no_of_tasks);
                else
                    Chromosome_MFGBO_new.skill_factor = population(i).skill_factor;
                end

                sf = Chromosome_MFGBO_new.skill_factor;

                % 函数值评价
                [Chromosome_MFGBO_new, calls_per_individual(i)] = evaluate(Chromosome_MFGBO_new, Tasks, 0, no_of_tasks, options);

                % 更新最优，锦标赛选择
                if Chromosome_MFGBO_new.factorial_costs(sf) < population(i).factorial_costs(sf)
                    population(i) = Chromosome_MFGBO_new;
