...
package momfo.qualityIndicator.fastHypervolume;
...
public class FastHypervolume {
	...
	public void computeHVContributions(SolutionSet solutionSet, int taskId) {
		int start = solutionSet.get(0).getProblemSet().get(taskId)
				.getStartObjPos();
		int end = solutionSet.get(0).getProblemSet().get(taskId).getEndObjPos();

		SolutionSet pop = new SolutionSet(solutionSet.size());

		for (int i = 0; i < solutionSet.size(); i++) {
			Solution sol = new Solution(end - start + 1);
			for (int j = start; j <= end; j++)
				sol.setObjective(j - start, solutionSet.get(i).getObjective(j));
			pop.add(sol);
		}

		computeHVContributions(pop);

		for (int i = 0; i < solutionSet.size(); i++)
			solutionSet.get(i).setCrowdingDistance(
					pop.get(i).getCrowdingDistance());
	}
