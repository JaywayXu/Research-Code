...
class BenchmarkFunctions:
    ...
    def f3_obj(self, x):
        '''Zakharov (Unimodal)'''
        z = np.power(x, 2).sum() + pow((x.sum()*0.5), 2)+pow((x.sum()*0.5), 4)
        return z
    ...
    def f4_obj(self, x):
        ...
        ff = np.zeros([D-1, 1])
        for i in range(D-1):
            ff[i] = 100 * np.power((np.power(x[i], 2)-x[i+1]),
                                   2)+np.power((x[i]-1), 2)
        z = ff.sum()
        return z
    ...
    def f7_obj(self, x):
        ...
        for i in range(D):
            if i == D-1:
                f[i] = 0.5 + (pow(np.sin(np.sqrt(pow(x[i], 2) + pow(x[0], 2))),
                                  2) - 0.5) / pow(1 + 0.001*(pow(x[i], 2) + pow(x[0], 2)), 2)
            else:
                f[i] = 0.5 + (pow(np.sin(np.sqrt(pow(x[i], 2) + pow(x[i+1], 2))),
                                  2) - 0.5) / pow(1 + 0.001*(pow(x[i], 2) + pow(x[i+1], 2)), 2)
        z = f.sum()
        return z
