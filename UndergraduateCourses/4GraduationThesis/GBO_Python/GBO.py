...
class GBO:
    ...
    def get_resault(self):
        ...
        for it in range(self.MaxIt):
            ...
            for i in range(self.nP):
                ...
                Xm = (self.X[r1, :]+self.X[r2, :] +
                      self.X[r3, :]+self.X[r4, :])/4
                ...
                Flag = 1
                GSR = self.GradientSearchRule(
                    ro1, Best_X, Worst_X, self.X[i, :], self.X[r1, :], DM, eps, Xm, Flag)
                DM = np.random.rand() * ro * (Best_X-self.X[r1, :])
                ...
                for j in range(self.nV):
                    ...
                    Xnew[0, j] = ra * (rb * X1[0, j] + (1-rb) *
                                       X2[0, j]) + (1-ra) * X3  # Eq.(27)
                ...
                if np.random.rand() < self.pr:
                    ...
                    if u1 < 0.5:
                        Xnew = Xnew + f1 * (u1*Best_X - u2*Xp) + f2 * ro * (
                            u3 * (X2-X1) + u2 * (self.X[r1, :] - self.X[r2, :])) / 2
                    else:
                        Xnew = Best_X + f1 * \
    ...
    def GradientSearchRule(self, ro1, Best_X, Worst_X, X, Xr1, DM, eps, Xm, Flag):
        ...
        if Flag == 1:
            ...
            Xs = Best_X - GSR + DM
        yp = np.random.rand() * (0.5 * (Xs+X) + np.random.rand() * DelX)  # Eq.(22.6)
        yq = np.random.rand() * (0.5 * (Xs+X) - np.random.rand() * DelX)  # Eq.(22.7)
        GSR = np.random.randn() * ro1 * (2*DelX*X) / (yp-yq+eps)  # Eq.(23)
        return GSR
