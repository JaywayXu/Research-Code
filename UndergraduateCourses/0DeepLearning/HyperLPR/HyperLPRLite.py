...
class LPR():  # 定义LPR大类，整个大类在main.py中都会运行
    ...
    def detectPlateRough(self, image_gray, resize_h=720, en_scale=1.08, top_bottom_padding_rate=0.05):  # 图像初步处理，以适应神经网络要求
        if top_bottom_padding_rate > 0.2:
            print("error:top_bottom_padding_rate > 0.2:",
        ...
        image_gray = cv2.cvtColor(
            image_color_cropped, cv2.COLOR_RGB2GRAY)  # rgb图像，Mat类型
    ...
    def model_seq_rec(self, model_path):  # 定义车牌初步定位函数，使用卷积神经网络和循环神经网络
        ...
        x = Reshape(target_shape=(int(conv_shape[1]), int(
            conv_shape[2] * conv_shape[3])))(x)
        x = Dense(32)(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        gru_1 = GRU(rnn_size, return_sequences=True,
                    kernel_initializer='he_normal', name='gru1')(x)  # 双层的双循环神经网络，正向
        gru_1b = GRU(rnn_size, return_sequences=True, go_backwards=True,
                     kernel_initializer='he_normal', name='gru1_b')(x)  # 反向
