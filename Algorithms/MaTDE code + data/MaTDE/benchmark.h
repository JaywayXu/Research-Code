...
double Ackley(double y[], int task_id) //Ackley
{
	...
	double s = 0.0;
	s = -20.0 * exp(-0.2 * sqrt(s_a / double(MAX_NVARS))) - exp(s_b / double(MAX_NVARS)) + 20.0 + exp(1.);
...
double Weierstrass50D(double y[], int task_id) //Weierstrass with MAX_NVARS dimensions
{
	double z[MAX_NVARS], tmp[MAX_NVARS];
	double LB = -0.5, UB = 0.5;
	for (int i = 0; i < MAX_NVARS; i++)
