...
gene population[total_task_num][4 * POPSIZE], newpopulation, mixpopulation[2 * POPSIZE];
...
void get_Cov_Inv(int trans_target)
{
	...
	for (k = 0; k <= NVARS - 1; k++){
		...
		for (j = 0; j <= NVARS - 1; j++)
			if (j != k) Cov_Inv[trans_target][k][j] *= Cov_Inv[trans_target][k][k];
		for (i = 0; i <= NVARS - 1; i++)
			if (i != k)
				for (j = 0; j <= NVARS - 1; j++)
					if (j != k){
...
void production(int task)
{
	...
	if (randval(0, 1) > alpha){   //perform the crossover and mutation within the subpopulation
		for (i = 0; i < POPSIZE; i++){
		...
		for (i = 0; i < POPSIZE; i++){
			...
			for (j = 0; j < MAX_NVARS; j++){
				if (k == j || randval(0, 1) < CR){            //at least one dimension is replaced
...
void final_report()
{
	...
	for (i = 0; i < task_num; i++){
		sprintf_s(name, "Result\\30 RUNS results for task %d.txt", i + 1);
		fopen_s(&pf, name, "w");
		for (j = 0; j < RUNS; j++){
			if (j < RUNS - 1) fprintf(pf, "%lf,", fbest_value[i][MAX_GEN / RECORD_FRE][j]);
			else  fprintf(pf, "%lf", fbest_value[i][MAX_GEN / RECORD_FRE][j]);
		}
		fclose(pf);
	}
	...
	for (i = 0; i < task_num; i++){
		sprintf_s(name, "Result\\TLC record for task%d.txt", i + 1);
		fopen_s(&pf, name, "w");
		for (j = 0; j < MAX_GEN / RECORD_FRE; j++){
			fprintf(pf, "GEN %d ~ %d: ", j * RECORD_FRE, (j + 1) * RECORD_FRE);
			for (l = 0; l < task_num; l++){
				if (l != i) fprintf(pf, "task%d: %lf\t", l + 1, (double)TLC_record[i][j][l] / RUNS);	
			}
			fprintf(pf, "\n");
		}
		fclose(pf);
