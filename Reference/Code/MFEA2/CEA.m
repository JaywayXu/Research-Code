function data_CEA = CEA(Tasks, pop, gen, selection_process, p_il, reps)
    % CEA function: implementation of single tasking CEA algorithm
    % Herein, the rmp = 0 and is fixed throughout the optimization process.
    ...
    if no_of_tasks <= 1
        error('At least 2 tasks required for CEA');
    end
    ...
    fid1 = fopen('final_CEA.txt', 'w'); % prints final objective values for each independent run
    ...
    for rep = 1:reps
        ...
        rmp = 0; % fixed for all generations
        generation = 1;
        ...
        while generation < gen
            ...
            disp(['CEA Generation = ', num2str(generation), ' best factorial costs = ', num2str(bestobj), '  rmp = ', num2str(rmp), ]); %
    ...
    data_CEA.wall_clock_time = toc;
    data_CEA.EvBestFitness = EvBestFitness;
    data_CEA.bestInd_data = bestInd_data;
    data_CEA.TotalEvaluations = TotalEvaluations;
