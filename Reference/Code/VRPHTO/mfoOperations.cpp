...
void factorial_rank(int size) {
    for (int i = 0; i < TASK_NUMBER; i++) {
        ...
        for (int j = 0; j < size; j++) chro[j].fRank[i] = j + 1;
    }
}
...
void skill_factor(int startPoint, int size) {
    ...
    for (int i = startPoint; i < size; i++) {
        ...
        if (min_index == VRP_T2) chro[i].count = vNumber[VRP_T2] - 1;
        if (min_index == VRP_T1) chro[i].count = vNumber[VRP_T1] - 1;
    }
}
    ...
    if (c.skill_factor == VRP_T1) {
        ...
        while (i < count) {
            ...
            for (j = i; j < count; j++) {
                ...
                tmp_count = add_element(tmp_seq, del.sequence[j], tmp_count - 1,
                                        tmp_count);
            }
