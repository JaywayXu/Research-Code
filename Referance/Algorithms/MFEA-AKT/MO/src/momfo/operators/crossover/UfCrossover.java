 ...
 * solutions.
 */
public class UfCrossover extends Crossover {
	/**
	...
	public UfCrossover(HashMap<String, Object> parameters) {
		super(parameters);
...
/*		int dem1=problemSet.get(parent1.getSkillFactor()).getNumberOfVariables();
		...
		if (PseudoRandom.randDouble() <= probability) {
			...
			for(i=0;i<dim;i++){
				if(r.nextBoolean())
					offs1.setValue(i, x1.getValue(i));
				else
					offs1.setValue(i, x2.getValue(i));
				
				if(r.nextBoolean())
					offs2.setValue(i, x2.getValue(i));
				else
