...
void split_T1(delimiter_individual *del_indi, int *seq)
{
	...
	for(int i=1;i<vNumber[VRP_T1];i++){
		...
		while(j<vNumber[VRP_T1]&&load<=max_capacity[VRP_T1]&&route_time<=customer_T1[DEPOT].due_date&&service_time<=customer_T1[selected].due_date){
			selected=sequence[j];
			...
			if(load<=max_capacity[VRP_T1]&&route_time<=customer_T1[DEPOT].due_date&&service_time<=customer_T1[sequence[j]].due_date){
				if(v[i-1]+cost<v[j]){
					v[j]=v[i-1]+cost;
					p[j]=i-1;
				}
...
void split_T2(delimiter_individual *del_indi, int *seq)
{
	...
	for(int i=1;i<vNumber[VRP_T2];i++){
		...
		while(j<vNumber[VRP_T2]&&load<=max_capacity[VRP_T2]&&route_time<=customer_T2[DEPOT].due_date&&service_time<=customer_T2[selected].due_date){
			selected=sequence[j];
			...
			if(i==j){
				...
				cost=choose_a_type_T2(load,tmp_seq,tmp_count);
				arrived_time=leave_time+travel_time_T2[sequence[j-1]][sequence[j]];
			}

			wait_time=customer_T2[sequence[j]].ready_time>arrived_time?(customer_T2[sequence[j]].ready_time-arrived_time):0;
			service_time=arrived_time+wait_time;
			leave_time=service_time+customer_T2[sequence[j]].service_time;
			route_time=leave_time+travel_time_T2[DEPOT][sequence[j]];

			if(load<=max_capacity[VRP_T2]&&route_time<=customer_T2[DEPOT].due_date&&service_time<=customer_T2[sequence[j]].due_date){
				if(v[i-1]+cost<v[j]){
					v[j]=v[i-1]+cost;
					p[j]=i-1;
				}
	...
	caculate_service_time_T2(del_indi->sequence,del_indi->service_time,del_indi->count);
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////new
...
void caculate_service_time_T2(int *sequence,float *stime,int count){
	...
	for(int i=0;i<count;i++){
		...
		arrived_time=travel_time_T2[sequence[i-1]][sequence[i]]+leave_time;
		wait_time=customer_T2[sequence[i]].ready_time>arrived_time?(customer_T2[sequence[i]].ready_time-arrived_time):0;
		service_time=arrived_time+wait_time;
...
bool evaluate_service_time_T2(int *sequence,float *stime,int count){
	for(int i=0;i<count;i++)
		if(stime[i]>customer_T2[sequence[i]].due_date)
			return false;
	return true;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////new
...
bool evaluate_vw_T1(int type, int *temseq, int count){
	...
	for(int j=1;j<count;j++){
		...
		wait_time=customer_T1[temseq[j]].ready_time>arrived_time?(customer_T1[temseq[j]].ready_time-arrived_time):0;
		service_time=arrived_time+wait_time;
		leave_time=service_time+customer_T1[temseq[j]].service_time;		
		if(service_time>customer_T1[temseq[j]].due_date)
			return false;
	}
	if(arrived_time<=vehicle_type_T1[type].vltime&&arrived_time<=customer_T1[DEPOT].due_date){
		return true;
	}
	else
		return false;
...
bool evaluate_vw_T2(int type, int *temseq, int count){
	...
	for(int j=1;j<count;j++){
		...
		wait_time=customer_T2[temseq[j]].ready_time>arrived_time?(customer_T2[temseq[j]].ready_time-arrived_time):0;
		service_time=arrived_time+wait_time;
		leave_time=service_time+customer_T2[temseq[j]].service_time;		
		if(service_time>customer_T2[temseq[j]].due_date)
			return false;
	}
	if(arrived_time<=vehicle_type_T2[type].vltime&&arrived_time<=customer_T2[DEPOT].due_date){
		return true;
	}
	else
		return false;
