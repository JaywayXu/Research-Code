...
int main(void)
{
	...
	for (int N = 0; N < FNUMBER; N++) 
	{
		...
		max_capacity[VRP_T1]=vehicle_type_T1[type_number[VRP_T1]-1].capacity;
		...
		for(int i=0;i<vNumber[VRP_T1];i++)
			for(int j=0;j<vNumber[VRP_T1];j++)
				if(i==j)
					...
					travel_time_T1[i][j]=sqrt(pow(abs(customer_T1[i].x-customer_T1[j].x),2)+pow(abs(customer_T1[i].y-customer_T1[j].y),2));
		...
		max_capacity[VRP_T2]=vehicle_type_T2[type_number[VRP_T2]-1].capacity;
		...
		for(int i=0;i<vNumber[VRP_T2];i++)
			for(int j=0;j<vNumber[VRP_T2];j++)
				if(i==j)
					...
					travel_time_T2[i][j]=sqrt(pow(abs(customer_T2[i].x-customer_T2[j].x),2)+pow(abs(customer_T2[i].y-customer_T2[j].y),2));
			...
			for(int phase=1;phase<=NP;phase++){			
				...
				while(task_eval[VRP_T1]!=ENUMBER||task_eval[VRP_T2]!=ENUMBER){	
					...
					if(task_eval[c1.skill_factor]<ENUMBER){
						...
						current_size++;
						if(c1.fitness[c1.skill_factor]<bestfitness[c1.skill_factor]){
							bestfitness[c1.skill_factor]=c1.fitness[c1.skill_factor];
							printf("c1.fitness[%d]:%f\n",c1.skill_factor,c1.fitness[c1.skill_factor]);
						}
					...
					if(task_eval[c2.skill_factor]<ENUMBER){
						...
						current_size++;				
						if(c2.fitness[c2.skill_factor]<bestfitness[c2.skill_factor]){
							bestfitness[c2.skill_factor]=c2.fitness[c2.skill_factor];
							printf("c2.fitness[%d]:%f\n",c2.skill_factor,c2.fitness[c2.skill_factor]);
						}
						if(current_size==T_SIZE)
							current_size=POP_SIZE;	
					}
					...
					if(c1.skill_factor==VRP_T1&&task_eval[VRP_T1]<ENUMBER){
						fprintf(fw_T1, "%d ", total_eval[VRP_T1]);
						fprintf(fw_T1, "%0.2f\r\n",bestfitness[VRP_T1]);
						sum_fitness[VRP_T1][total_eval[VRP_T1]]+=bestfitness[VRP_T1];
						task_eval[VRP_T1]++;
						total_eval[VRP_T1]++;
					}
					if(c1.skill_factor==VRP_T2&&task_eval[VRP_T2]<ENUMBER){
						fprintf(fw_T2, "%d ", total_eval[VRP_T2]);
						fprintf(fw_T2, "%0.2f\r\n",bestfitness[VRP_T2]);
						sum_fitness[VRP_T2][total_eval[VRP_T2]]+=bestfitness[VRP_T2];
						task_eval[VRP_T2]++;
						total_eval[VRP_T2]++;										
					}

					if(c2.skill_factor==VRP_T1&&task_eval[VRP_T1]<ENUMBER){
						fprintf(fw_T1, "%d ", total_eval[VRP_T1]);
						fprintf(fw_T1, "%0.2f\r\n",bestfitness[VRP_T1]);
						sum_fitness[VRP_T1][total_eval[VRP_T1]]+=bestfitness[VRP_T1];
						task_eval[VRP_T1]++;
					...
					if(c2.skill_factor==VRP_T2&&task_eval[VRP_T2]<ENUMBER){
						fprintf(fw_T2, "%d ", total_eval[VRP_T2]);
						fprintf(fw_T2, "%0.2f\r\n",bestfitness[VRP_T2]);
						sum_fitness[VRP_T2][total_eval[VRP_T2]]+=bestfitness[VRP_T2];
						task_eval[VRP_T2]++;
