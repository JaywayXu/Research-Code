...
void ordered_xover(undelimiter_individual *c1,undelimiter_individual *c2,undelimiter_individual *p1, undelimiter_individual *p2)
{
	...
	int length =vNumber[VRP_T1]>vNumber[VRP_T2]?vNumber[VRP_T1]-1:vNumber[VRP_T2]-1;
...
void mutation(undelimiter_individual *p,undelimiter_individual *c){
	...
	count=vNumber[VRP_T1]>vNumber[VRP_T2]?vNumber[VRP_T1]-1:vNumber[VRP_T2]-1;
...
void rebuilt_chrom(undelimiter_individual *c,delimiter_individual *del_c){
	int j=0;
	int maxNumber=vNumber[VRP_T1]>vNumber[VRP_T2]?vNumber[VRP_T1]:vNumber[VRP_T2];
	maxNumber-=1;
	...
	while(1){
		...
		for(int i=0;i<(count-1);i++){			
			...
			for(int j=0;j<(count-1);j++){
				...
				if(evaluate_service_time_T2(sequence,service_time,count)&&evaluate_load_T2(sequence,count)){
