 ...
 * solutions.
 */
public class GeoCrossover extends Crossover {
	/**
	 ...
	 */
	private String name = "Geometrical";	
	public GeoCrossover(HashMap<String, Object> parameters) {
		super(parameters);
...
/*		int dem1=problemSet.get(parent1.getSkillFactor()).getNumberOfVariables();
		...
		if (PseudoRandom.randDouble() <= probability) {
			...
			for(i=0;i<dim;i++){
				...
				insertValue=Math.pow(x1.getValue(i), lambda)*Math.pow(x2.getValue(i), 1-lambda);
				if (insertValue < yL)
					insertValue = yL;
				...
				insertValue=Math.pow(x2.getValue(i), lambda)*Math.pow(x1.getValue(i), 1-lambda);
				if (insertValue < yL)
					insertValue = yL;
